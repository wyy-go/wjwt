...
const identityKey = "wjwt-identify"
...
func (sf *Signature) Encode(account *Account, timeouts ...time.Duration) (string, time.Time, error) {
	timeout := sf.timeout
...
func (sf *Signature) Decode(token string) (*Account, error) {
...
func (sf *Signature) CheckTokenExpire(token *jwt.Token, err error) (*Account, error) {
	if err != nil {
...
func FromAccount(c *gin.Context) *Account {
	account, _ := c.Get(identityKey)
	return account.(*Account)
}

func SetAccount(c *gin.Context, account *Account) {
	c.Set(identityKey, account)
}
